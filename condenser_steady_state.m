...
K0= 12.87*vw^5 - 145.1*vw^4 + 638.4*vw^3 - 1525*vw^2 + 3007*vw + 714.7;%流速单位为m/s,K0单位为W/m^2*k;
Fw= 1.151*exp(-((tw-55.95)/75.15)^2) + 0.07026 *exp(-((tw-20.01)/14.15)^2);%水温单位为摄氏度  
Fm= (0.002362*L1^2 - 0.2093*L1 + 4.056) / (L1 + 3.863);%壁厚单位为mm
Fc=0.9;
...
Q=Dc*(H1-H2);%流量kg/s，KJ/(Kg)，KJ/(m^2*K*s），
...
%H2=refpropm('H', 'T',ts+273.15,'Q',0, 'water')*0.001;
Q=Dc*(H1-H2);%流量kg/s，KJ/(Kg*K)，KJ/(m^2*K*s），
tw1=Dc*(H1-H2)/(cpw*mw)+tw;
