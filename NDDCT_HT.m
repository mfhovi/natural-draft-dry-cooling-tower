...
v02=1.55;
Twod=(Tin*3+Ta6)/4; % Estimation of outlet water temperature in dry cooling section
i=1;
p=1;
m=1;
n=1;
q=1;
delta_pf=0.51;
while abs(delta_pf)>=0.5 % outer loop calculation
if delta_pf>0
...
delta_fl_bad=0.21;
while abs(delta_fl_bad)>=0.2 % Dry cooling section power balance
if delta_fl_bad>0
...
f=power(1.82*log10(Rewd)-1.64,-2);%Darcy drag coefficient, water side
Nuwd=(f/8)*(Rewd-1000)*Prwd*(1+power(d/Lte,2/3))/(1+12.7*power(f/8,0.5)*(power(Prwd,2/3)-1));% Nu for water side heat transfer
hw=Nuwd*Lamdawd/d;%Heat transfer coefficient at water side for Nu
